# -*- coding: utf-8 -*-
"""
Created on Thu Jul  5 12:04:20 2018
...
k1 = int(k1)
def lsc_code(data,lab,neigh,k2):
    clss = np.unique(lab)
    dim = data.shape[1]
    ...
    for cl in clss:
        ...
        for xi in data[lab == cl]:
            distss = []
            for xk in data[lab == cl]:
                dist = LA.norm(xi - xk)
                distss.append(dist)
            distance_frame = pd.DataFrame(data={"distance": distss, "idx": distss.index})
            distance_frame.sort_values('distance',inplace = True)
            k_nearest_index = distance_frame.iloc[1:neigh+1]['idx']
            k_nearest = data[lab == cl][k_nearest_index.index]
    ...
    for cl in clss:
        ...
        for xi in data[lab == cl]:  
            ...
            distance_frame = pd.DataFrame(data={"distance": distss, "idx": distss.index})
            distance_frame.sort_values('distance',inplace = True)            
            k_nearest_index = distance_frame.iloc[0:neigh]['idx']
            k_nearest = data[lab != cl][k_nearest_index.index]
    ...
    f = tr_data.dot(W)
    return f,W
tr_data_lsc, red_fac = lsc_code(tr_data,tr_lab,neigh1,k1)
...
acc = svmclassifier(tr_data_lsc,tr_lab,te_data_lsc,te_lab)
