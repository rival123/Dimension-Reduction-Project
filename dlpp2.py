# -*- coding: utf-8 -*-
"""
Created on Tue Jul 10 11:08:36 2018
...
k1 = int(k1)
def dlpp_code(data,lab,neigh,k2):
    clss = np.unique(lab)
    dim = data.shape[0]
    w = np.zeros((dim,dim))
    d = np.zeros((dim,dim))
    ...
    for cl in clss:
        ...
        for j in range(len(data[lab == cl])):
            distss = []
            for l in range(len(data[lab == cl])):
                dist = LA.norm(data[lab == cl][j] - data[lab == cl][l])
                distss.append(dist)
            ...
            for k in range(len(data[lab == cl])):
                if np.any(k_nearest == data[lab == cl][k]):
    ...
    new_d = tr_data.dot(W)
    return new_d,W
tr_data_dlpp, red_fac = dlpp_code(tr_data,tr_lab,neigh1,k1)
